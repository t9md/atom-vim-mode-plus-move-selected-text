...
module.exports = {
  ...
  consumeVim({Base, observeVimStates}) {
    const {commands, stateManager} = require("./move-selected-text")(Base)

    for (const name in commands) {
      const klass = commands[name]
      this.subscriptions.add(klass.registerCommand())
    }

    observeVimStates(vimState =>
      vimState.modeManager.onDidDeactivateMode(({mode}) => mode === "visual" && stateManager.delete(vimState.editor))
    )
  },
}
